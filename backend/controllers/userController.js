...
exports.resetPassword = catchAsyncError(async function (req, res, next) {
  ...
  if (req.body.password !== req.body.confirmPassword) {
    return next(
      new ErrorHandler("confirmPassword should be same as Password ")
    );
  }
...
exports.updateProfile = catchAsyncError(async function (req, res, next) {
  ...
  if(req.body.avatar!==""){
    const ourUsers=await user.findById(req.user.id);
    const imageId=ourUsers.avatar.public_id;
